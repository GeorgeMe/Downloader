package com.yunyan.downloader.service;

import android.content.Context;
import android.content.Intent;
import android.util.Log;

import com.yunyan.downloader.db.IThreadDAO;
import com.yunyan.downloader.db.ImpThreadDAO;
import com.yunyan.downloader.entity.FileInfo;
import com.yunyan.downloader.entity.ThreadInfo;
import com.yunyan.downloader.util.BufferedRandomAccessFile;
...
//			downloadThread.start();
	...
	private synchronized void checkAllThreadsFinished(){
		...
		if(allFinished){
			Intent intent = new Intent(com.yunyan.downloader.service.DownloadService.ACTION_FINISH);
				...
				if (206 == conn.getResponseCode()) {
					...
					while ((len = bis.read(buffer, 0, buffer.length)) > 0) {
						...
						if (System.currentTimeMillis() - time > (1 * 1000) ) {
							...
							Log.e("DownloadTask", "发送广播,下载文件ID="+mFileInfo.getId() + "下载进度为"+100 * mFinished / mFileInfo.getLength());
						}
						// 模拟网速很慢
						Thread.sleep(500);
...
//					检查是否所有线程下载完毕
					...
					Intent intent = new Intent(DownloadService.ACTION_EXCEPTION);
				...
				if (e instanceof ConnectException) {
					...
					Intent intent = new Intent(DownloadService.ACTION_EXCEPTION);
					intent.putExtra(Const.EXCEPTION_KEY, "UNKNOWN ERROR");
					intent.putExtra(Const.ID_KEY, mFileInfo.getId());
					// 发送广播
					mContext.sendBroadcast(intent);
					//Log.e(TAG, "未知错误", e);
				...
				try {
					...
					Intent intent = new Intent(DownloadService.ACTION_EXCEPTION);
					intent.putExtra(Const.EXCEPTION_KEY, e.getMessage());
					intent.putExtra(Const.ID_KEY, mFileInfo.getId());
					// 发送广播
					mContext.sendBroadcast(intent);
					e.printStackTrace();
